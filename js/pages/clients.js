...
class ClientsPage {
    ...
    class SalesPage {
                ...
                <div class="sales-grid">
                    <div class="card sale-builder">
                        ...
                        <div class="sale-form">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="sale-cantidad" min="1" placeholder="Ingrese cantidad" required>
                            </div>
                            <div class="form-group">
                                <label class="form-label required" for="sale-precio">Precio Venta</label>
                                <input type="text" id="sale-precio" placeholder="Precio sugerido" required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-actions">
                                <button class="btn btn-primary" id="btn-add-item">${ICONS.plus}<span>Agregar producto</span></button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card sale-summary">
                        ...
                        <div class="summary-actions">
                            <button class="btn btn-ghost" id="btn-clear-sale">Limpiar venta</button>
        ...
        populateBrandSelect() {
            ...
            select.innerHTML = `<option value="">Seleccione marca</option>` + brands.map(b => `<option value="${b}">${b}</option>`).join('');
        }
        ...
        populateAmperajeSelect() {
            ...
            select.innerHTML = `<option value="">Seleccione amperaje</option>` + amperajes.map(a => `<option value="${a}">${a}</option>`).join('');
        }
        ...
        getCartQuantityForProduct(productId) {
            return this.items.filter(i => i.productId === productId).reduce((sum, item) => sum + item.cantidad, 0);
        }
        ...
        addItemToSale() {
            ...
            // Reset cantidad, mantener precio sugerido
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="table-empty">
                                ...
                                <div class="table-empty-text">Selecciona marca y amperaje y presiona "Agregar producto"</div>
                            </div>
                        </td>
                    </tr>
                `;
                ...
                <tr data-index="${index}">
                    ...
                    <td class="col-actions">
                        <div class="row-actions">
                            <button class="row-action-btn delete" data-action="remove" title="Eliminar">${ICONS.trash}</button>
                        </div>
                    </td>
                </tr>
            `).join('');
        ...
        updateSummary() {
            ...
            const descuento = this.discount;
            const saldo = Math.max(0, totalImporte - descuento);
        ...
        renderSalesTable() {
            ...
            const totalPages = Math.max(1, Math.ceil(this.sales.length / this.itemsPerPage));
            if (this.currentPage > totalPages) this.currentPage = totalPages;
            const start = (this.currentPage - 1) * this.itemsPerPage;
            const end = start + this.itemsPerPage;
            const pageData = this.sales.slice(start, end);

            tbody.innerHTML = pageData.map(sale => {
                const fecha = new Date(sale.date);
                const formattedDate = fecha.toLocaleDateString('es-BO', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                return `
                    <tr data-id="${sale.id}">
                        <td>${formattedDate}</td>
                        ...
                        <td class="col-actions">
                            <div class="row-actions">
                                <button class="row-action-btn view" data-action="view" data-id="${sale.id}" title="Ver detalle">${ICONS.eye}</button>
                            </div>
                        </td>
                    </tr>
                `;
            ...
            pagination.innerHTML = `
                <div class="pagination-info">Mostrando ${start + 1} - ${Math.min(end, this.sales.length)} de ${this.sales.length} ventas</div>
                <div class="pagination-controls">
                    <button class="pagination-btn" id="sales-btn-prev" ${this.currentPage === 1 ? 'disabled' : ''}>◀</button>
                    <div class="pagination-pages">${pagesHtml}</div>
                    <button class="pagination-btn" id="sales-btn-next" ${this.currentPage === totalPages ? 'disabled' : ''}>▶</button>
                </div>
            `;
        ...
        showSaleDetail(sale) {
            ...
            Modal.open({
                ...
                size: 'default',
                showFooter: true,
                footerContent: `<button class="btn btn-primary" onclick="Modal.closeAll()">Cerrar</button>`
            });
        }
